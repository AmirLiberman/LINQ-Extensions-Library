...
namespace LinqExtTests.Operators
{
  [TestClass()]
  public class StdevTests
  {
    ...
    {
      IEnumerable<Tuple<string, double>> source = Enumerator.Generate(7, X => Generator.GenerateDouble(X, 10));
    ...
    {
      IEnumerable<Tuple<string, double?>> source = Enumerator.Generate(7, X => Generator.GenerateNullableDouble(X, 10, 10));
      double? expected = 2.160;
      double? actual;
