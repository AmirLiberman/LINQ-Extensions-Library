...
using System.Linq;
using LinqExtensions.Operators;

namespace LinqExtensions.Sequence
{
  /// <summary>
  ...
  public static class Enumerator
  {
    ...
    {
      Utilities.ValidateIsNotNull(generator, nameof(generator));
    ...
    {
      Utilities.ValidateTimespan(step, nameof(step));
    ...
    {
      Utilities.ValidateIsNumeric<T>(nameof(Generate));
    ...
    {
      Utilities.ValidateIsNumeric<T>(nameof(Range));
    ...
    {
      Utilities.ValidateIsNumeric<T>(nameof(Random));
    ...
    {
      Utilities.ValidateIsNotNull(input1, nameof(input1));
      Utilities.ValidateIsNotNull(input2, nameof(input2));
    ...
    {
      Utilities.ValidateIsNotNull(source, nameof(source));
      Utilities.ValidateIsNotNull(selector, nameof(selector));
      Utilities.ValidateSequenceIsNotEmpty(source, "source");
    ...
    public static TSource ElementAtMax<TSource, TSelector>(this IEnumerable<TSource> source, Func<TSource, TSelector> selector) where TSelector : IComparable
    {
      Utilities.ValidateIsNotNull(source, nameof(source));
      Utilities.ValidateIsNotNull(selector, nameof(selector));
      Utilities.ValidateSequenceIsNotEmpty(source, nameof(source));
    ...
    /// <summary>
    /// Returns the element that is closest the average of the values returned by the selector function. 
    /// </summary>
    /// <typeparam name="TSource">The type of the elements of the source sequence.</typeparam>
    ...
    /// <returns>The element that is closest the average of the values returned by the selector function.</returns>
    ...
    public static TSource ElementAtAverage<TSource, TSelector>(this IEnumerable<TSource> source, Func<TSource, TSelector> selector, AverageMatchType matchType)
    {
      ...
      Utilities.ValidateIsNotNull(selector, nameof(selector));
      TSource[] sourceArr = source.ToArray();
      Utilities.ValidateSequenceIsNotEmpty(sourceArr, nameof(source));
      Utilities.ValidateIsNumeric<TSelector>(nameof(ElementAtAverage));
      ...
      switch (matchType)
      {
        ...
        case AverageMatchType.ExactOrSmaller:
          return minElement;
        default:
          throw Exceptions.MatchType(matchType);
      }
    }
    ...
    public static IEnumerable<TSource> ElementsWithinStdDev<TSource>(this IEnumerable<TSource> source, Func<TSource, double> selector, double deviations)
    {
      ...
      TSource[] sourceArr = source.ToArray();
      Utilities.ValidateSequenceIsNotEmpty(sourceArr, "source");
      Utilities.ValidateValueIsInRange(deviations, 0, double.MaxValue, "deviations");
