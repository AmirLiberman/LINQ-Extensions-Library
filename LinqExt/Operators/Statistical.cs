  ...
  public static class Statistical
  {
    ...
    public static IEnumerable<KeyValuePair<TSource, int>> Frequency<TSource>(this IEnumerable<TSource> source)
    {
      ...
      return source.GroupBy(I => I)
                   .Select(K => new KeyValuePair<TSource, int>(K.Key, K.Count()));
    }
    ...
    {
      Utilities.ValidateIsNotNull(source, nameof(source));
      Utilities.ValidateIsNotNull(buckets, nameof(buckets));
      Utilities.ValidateIsNotNull(bucketSelector, nameof(bucketSelector));

      IEnumerable<KeyValuePair<TBucket, int>> rawFreq = source.GroupBy(I => bucketSelector(I, buckets))
                                                              .Select(K => new KeyValuePair<TBucket, int>(K.Key, K.Count()));
    ...
    public static IEnumerable<KeyValuePair<TBucket, int>> Frequency<TSource, TBucket>(this IEnumerable<TSource> source, Func<TSource, TBucket> bucketSelector)
    {
      ...
      return source.GroupBy(I => bucketSelector(I))
                   .Select(K => new KeyValuePair<TBucket, int>(K.Key, K.Count()));
    }
    ...
    {
      Utilities.ValidateIsNotNull(source, nameof(source));
      Utilities.ValidateBlockSizeTwoOrMore(blockSize);
      ...
      using (IEnumerator<double> right = source.GetEnumerator())
      {
        ...
        // If a NaN is encountered, set the NaNs to the block size and yield NaNs until it clears.
    ...
    {
      Utilities.ValidateIsNotNull(source, nameof(source));
      Utilities.ValidateIsNotNull(weight, nameof(weight));
      Utilities.ValidateBlockSizeTwoOrMore(blockSize);
      ...
      using (IEnumerator<double> se = source.GetEnumerator())
      {
        ...
        {
          if (!se.MoveNext())
            throw Exceptions.ValueMinBlockSize(nameof(source));
    ...
    public static double Variance(this IEnumerable<double> source)
    {
      ...
      if (itemsCount < 2)
        throw Exceptions.SequenceMinTwo(nameof(source));      
      ...
      using (IEnumerator<decimal?> src = sourceArr.GetEnumerator())
        if (src.MoveNext())
        {
          ...
          while (src.MoveNext())
          {
            ...
            if (i <= 1)
              throw Exceptions.CumulativeVarianceZeroMembers();
    ...
    public static IEnumerable<double?> Variance(this IEnumerable<double?> source, int blockSize)
    {
      ...
      using (IEnumerator<double?> right = source.GetEnumerator())
      {
        ...
        while (block > 1)
        {
          ...
          {
            if (elements < 2)
              throw Exceptions.SequenceMinTwoNotNull(nameof(source));
